...
impl ReadReplicaManager {
    ...
    pub fn from_config(
        ...
        for path in &paths {
            ...
            {
                let _ = path;
                return Err(MemoryError::Storage("No storage backend available for replicas".into()));
            }
        }
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        let index = self.current_replica.fetch_add(1, std::sync::atomic::Ordering::Relaxed)
            % self.replicas.len();
