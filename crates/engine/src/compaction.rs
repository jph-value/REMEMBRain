...
impl MemoryCompactor {
    ...
    /// Compact memories
    pub fn compact(&self, memories: Vec<MemoryArtifact>) -> Result<(Vec<MemoryArtifact>, CompactionStats)> {
        ...
        if self.config.merge_same_entity {
            ...
            for memory in &memories {
                ...
                entity_groups.entry(entity_key).or_default().push(memory.clone());
            }
            ...
            for (entity_key, mut group) in entity_groups {
                if group.len() > 1 && group.len() <= self.config.max_merge_count {
                    ...
                    let should_merge = group.iter().all(|m| {
                        now.signed_duration_since(m.timestamp) > chrono::Duration::from_std(self.config.min_age).unwrap_or_default()
                    });
    ...
    fn merge_memories(&self, memories: &[MemoryArtifact]) -> Result<MemoryArtifact> {
        if memories.is_empty() {
            return Err(MemoryError::InvalidQuery("Cannot merge empty memory list".to_string()));
        }
    ...
    pub fn should_compact(&self) -> bool {
        ...
        match *last {
            Some(last_time) => {
                ...
                elapsed > chrono::Duration::from_std(self.config.compaction_interval).unwrap_or_default()
            }
